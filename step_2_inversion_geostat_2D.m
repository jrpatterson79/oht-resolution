...
for o = 1:1:num_obs
    ...
    tseries_err_est = (sum((tseries_noisy - M*obs_phasor).^2)./(num_t))^.5;
...
parfor a = 1 : num_alpha
    ...
    model_err(a,:) = norm(params_best(1:num_cells) - X(1:num_cells,1)*beta_best(1));
    data_err(a,:) = (y - h_function(params_best))' * R_inv * (y - h_function(params_best));
